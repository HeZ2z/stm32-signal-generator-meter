...
单板 `STM32F4/F7` 实践项目。当前仓库已经完成 `PWM 输出 + 输入测量 + UART/LCD/触摸` 闭环固件，并进入下一阶段规划：在保留旧主线可编译、可回归的前提下，继续扩展 `真实波形显示 + 双 DAC 输出 + 双通道采样 + XY/YT 显示` 演示链路。

## 项目状态

- 阶段：`M7 已完成，M8-M11 已规划`
- 开发板：`Apollo STM32 F4/F7`
- 当前硬件条件：`仅开发板本体，无外接传感器`
- 当前策略：按 `STM32F429` 路线收敛主固件，并在现有 PWM/测量主线上并行扩展 DAC/ADC 波形显示主线
- 当前未决项：按 `M8-M11` 推进真实波形显示、双通道输出与 `XY/YT` 演示
...
当前仓库还补充了误差样例和演示脚本文档，并把显示层升级为 `UART` 主通道 + `RGBLCD` 状态页后端 + `GT9xxx` 触摸调参入口。下一阶段会把 LCD 从状态页进一步推进到真实波形页，并逐步加入双路 DAC、双通道采样和 `XY/YT` 图形显示。
...
- 输入路径：当前为 `TIM PWM Input`，后续扩展 `ADC + DMA`
- 控制路径：`UART 命令 + GT9xxx 触摸按键`
- 显示路径：`UART` 主通道，`ALIENTEK 4342 RGBLCD` 已接入状态页与触摸界面，后续扩展 `YT / XY`
- 软件结构：
  - `signal_gen`：当前 PWM 信号产生
  - `signal_measure`：当前输入测量
  - `signal_gen_dac`：后续双 DAC 波形产生
  - `signal_capture_adc`：后续 ADC 采样
  - `scope_render`：后续 `YT / XY` 绘图
  - `ui_ctrl`：参数设置
