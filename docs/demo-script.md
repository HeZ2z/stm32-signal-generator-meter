# Demo Script

用于演示和答辩时按固定顺序展示系统能力，避免现场临时操作失误。当前默认主演示链路为 `M9/M10` 双 DAC / 双 ADC / LCD 双通道 `YT + 最小 XY`；`M11` 的 `sine + 李萨如` 三场景已具备代码路径，待实板验证后升级为正式主演示；旧 `PB6 -> PA7` 仅保留为 legacy 回归脚本。

## M9 演示前准备

- 固件已烧录到开发板
- 开发板正常从 Flash 启动
- 杜邦线连接 `PA4(DAC1) -> PA0(ADC1_IN0)`
- 杜邦线连接 `PA5(DAC2) -> PA6(ADC1_IN6)`
- 串口连接到 `/dev/ttyUSB0`
- LCD 已点亮并进入双通道波形页
- 打开串口监视：

```bash
sudo python3 tools/serial_monitor.py --port /dev/ttyUSB0 --baud 115200
```

## M9 演示步骤

1. 上电或复位后，说明系统已经完成 `双 DAC 输出 + 双 ADC 回采 + UART/LCD/触摸` 闭环。
2. 展示默认状态：

```text
DAC SQUARE freq=1000Hz | VIEW YT | ADC CH-A=LIVE CH-B=LIVE @100000Hz | LCD READY | TOUCH READY
```

1. 在 LCD 上触摸把频率调整到 `2000Hz`，说明系统已经从默认参数切换到新的目标输出。

2. 观察 LCD 顶部输入卡：
   - `CH-A PA4->PA0`
   - `CH-B PA5->PA6`
   两路都应显示真实 `F/D` 或在超窗时显示 `F=<当前输出频率> D=--`
3. 再通过 LCD 触摸把频率调整到 `5000Hz`。
4. 观察双 trace 随频率同步变化，且颜色和标签与实际接线一致。
5. 拔掉 `PA5 -> PA6`，展示 `CH-B` 退化为 `NO SIGNAL`，`CH-A` 继续正常。
6. 重新接回 `PA5 -> PA6`，说明系统可恢复到正常双通道状态。
7. 如需兜底演示，可在串口输入 `help`、`status`、`freq <hz>`，证明 `UART` 仍可作为控制入口。

## M10 演示步骤

1. 上电或复位后，说明当前主链路已经进入 `DAC -> ADC -> LCD` 的双通道真实采样模式，默认先看 `YT`。
2. 展示默认 `square` 输出与双通道 `YT` 叠加波形。
3. 通过 LCD 点击 `WAVE` 切换到 `triangle`，说明当前开放波形仅有 `square / triangle`。
4. 观察 `YT` 页面顶部卡片：
   - 方波在可估算窗口内显示真实 `F/D`
   - 三角波按当前 M10 设计退化为 `F=<当前输出频率> D=--`
5. 点击 `YT/XY` 进入 `XY` 页面，说明 `X=CH-A(PA0)`、`Y=CH-B(PA6)` 均来自真实 ADC 采样。
6. 说明当前双 DAC 输出仍固定同频同相，因此：
   - `square` 在 `XY` 上主要表现为退化斜线
   - `triangle` 在 `XY` 上主要表现为窄四边形或近平行四边形
7. 点击 `YT/XY` 返回 `YT`，证明两个页面可以稳定来回切换。
8. 如需串口兜底，输入 `help`、`status`、`freq <hz>`；若输入 `duty <1-99>`，系统只会提示当前 M10 DAC 模式不可调。

## M10 演示时建议说明

- 当前主链路不再依赖旧 `PWM + Input Capture`，而是 `DAC -> ADC -> LCD` 真值闭环。
- 当前 `XY` 页面是最小版：边框、中心线、逐点绘制，不含持久辉光、抗锯齿和双缓冲特效。
- 当前双通道输出仍固定同频同相，因此 M10 的 `XY` 主要用于证明真实轨迹页已经打通，而不是完整李萨如演示。
- 断线后对应输入卡会退化为 `NO SIGNAL`，不会保留旧值。
- 这套方案不依赖外部信号源，只需要单板和两根杜邦线即可完成完整演示。
- 推荐主演示频率固定为 `1000Hz`、`2000Hz`、`5000Hz`。
- 超出稳定估算窗口时，输入卡按设计显示 `F=<当前输出频率> D=--`。

## M10 失败回退方案

- 若串口没有输出，先检查是否正常从 Flash 启动、波特率是否为 `115200`。
- 若某一路输入卡无效，优先检查 `PA4 -> PA0` 或 `PA5 -> PA6` 杜邦线。
- 若出现 `ERR unknown command`，重新输入标准命令：`help`、`status`、`freq <hz>`；`duty <1-99>` 仅用于提示当前模式不可调。
- 若 LCD 可亮但触摸未恢复，优先回退到串口命令演示，不阻塞主链路说明。

## Legacy 回归脚本

### M7/M8 旧链路

- 接线：`PB6(TIM4_CH1) -> PA0(ADC1_IN0)`
- 演示目标：保留旧链路作为回归用，不再作为默认主演示

### M11 正弦波 + 李萨如（待实板确认）

**演示目标**：展示 `square / triangle / sine` 三种波形与 `YT / XY` 双模式

**推荐主演示参数**（需完成实板验证后作为正式主演示）：
- 同频同相（phase=0, ratio=1）→ 直线
- 同频 90°（phase=157, ratio=1）→ 圆/椭圆
- 1:2 频率比（phase=0, ratio=2）→ 8 字形

**观察点**：`XY` 模式可稳定展示直线、圆/椭圆和 1:2 李萨如图形

> 当前三组场景代码路径已完成，待实板验证后作为正式主演示。
