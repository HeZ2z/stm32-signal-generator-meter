...
| 2026-04-18 | `M8` | 真实采样 `YT` 波形显示规划 | 固定方案：`PB6(TIM4_CH1) -> PA0(ADC1_IN0)`，目标为单通道真实波形显示 | Planned | 当前仅完成文档落地，待实现 `ADC + DMA + LCD` 波形页 |
| 2026-04-18 | `M9` | 双 DAC 双方波规划 | 固定方案：`PA4(DAC1) -> PA0`、`PA5(DAC2) -> PA1`，目标为双通道 `YT` | Planned | 当前仅完成引脚与阶段定义，待实现双路 DAC 和双通道回采 |
